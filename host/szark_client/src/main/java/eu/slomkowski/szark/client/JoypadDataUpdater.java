...
public class JoypadDataUpdater 
{
	...
	{
		this.joy = joypadBackend;
		this.status = status;
	}

	private JoypadBackend joy;
	...
	public void update()
	{
		// movement 
		joy.poll();
		float fowBack = joy.getForwBackVal();
		float leftRight = joy.getLeftRightVal();
		
		// arm
		float armShoulder = joy.getShoulderVal();
		float armElbow = joy.getElbowVal();
		float armWrist = joy.getWristVal();
		float armGripper = joy.getGripperVal();
		
		byte motorSpeedLimit = status.motors.getSpeedLimit();
		
		//float v = Math.abs((float)( motorSpeedLimit * fowBack * (1.0f - Math.abs(leftRight)) ));
		...
		else if(fowBack > 0)
		{
			...
			if(leftRight < 0)
			{
				...
				status.motors.right.setSpeed((byte) (motorSpeedLimit * (1 - Math.abs(leftRight) * Math.abs(fowBack) )));
			}
			else if(leftRight > 0)
			{
				status.motors.right.setSpeed((byte) (motorSpeedLimit * Math.abs(fowBack)));
				status.motors.left.setSpeed((byte) (motorSpeedLimit * (1 - Math.abs(leftRight) * Math.abs(fowBack) )));
			}
			else
			{
				status.motors.left.setSpeed((byte) (motorSpeedLimit * Math.abs(fowBack)));
