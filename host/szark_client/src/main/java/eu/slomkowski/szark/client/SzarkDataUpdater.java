...
public class SzarkDataUpdater
{
	...
	private byte[] received = new byte[Hardcoded.MAX_DATA_LENGTH];
	 ...
	 *              , ConnectException,
	 */
	public synchronized void update() throws HardwareStoppedException,
			ConnectionErrorException
	{
		...
		try
		{
			...
			// SERVER COMMANDS
			if (status.server.mustShutdown()) send.append("Ss");
			if (status.server.mustExit()) send.append("Sx");
			if (status.server.mustReboot()) send.append("Sr");
			...
			else
			{
				...
				// left direction
				if (status.motors.left.getDirection() == SzarkStatus.Direction.FORWARD) send
						.append("Mdlf");
				else if (status.motors.left.getDirection() == SzarkStatus.Direction.BACKWARD) send
						.append("Mdlb");
				else send.append("Mdl0");
				// left speed
				send.append("Msl");
				send.append((char) ('a' + status.motors.left.getSpeed()));
				// right direction
				if (status.motors.right.getDirection() == SzarkStatus.Direction.FORWARD) send
						.append("Mdrf");
				else if (status.motors.right.getDirection() == SzarkStatus.Direction.BACKWARD) send
						.append("Mdrb");
				else send.append("Mdr0");
				// right speed
				send.append("Msr");
				send.append((char) ('a' + status.motors.right.getSpeed()));

				// LIGHTS
				if (status.lights.isHigh()) send.append("Lhe");
				else send.append("Lhd");
				if (status.lights.isLow()) send.append("Lle");
				else send.append("Lld");
				if (status.lights.isGripper()) send.append("Lge");
				else send.append("Lgd");
				if (status.lights.isCamera()) send.append("Lce");
				else send.append("Lcd");
				...
				{
					if (status.arm.shoulder.getDirection() == SzarkStatus.Direction.FORWARD) send
							.append("Adsf");
					else if (status.arm.shoulder.getDirection() == SzarkStatus.Direction.BACKWARD) send
							.append("MAdsb");
					else send.append("Ads0");
					send.append("Ass");
					send.append((char) ('a' + status.arm.shoulder
							.getSpeed()));

					if (status.arm.elbow.getDirection() == SzarkStatus.Direction.FORWARD) send
							.append("Adef");
					else if (status.arm.elbow.getDirection() == SzarkStatus.Direction.BACKWARD) send
							.append("MAdeb");
					else send.append("Ade0");
					send.append("Ase");
					send.append((char) ('a' + status.arm.elbow.getSpeed()));

					if (status.arm.wrist.getDirection() == SzarkStatus.Direction.FORWARD) send
							.append("Adwf");
					else if (status.arm.wrist.getDirection() == SzarkStatus.Direction.BACKWARD) send
							.append("MAdwb");
					else send.append("Adw0");
					send.append("Asw");
					send.append((char) ('a' + status.arm.wrist.getSpeed()));

					if (status.arm.gripper.getDirection() == SzarkStatus.Direction.FORWARD) send
							.append("Adgf");
					else if (status.arm.gripper.getDirection() == SzarkStatus.Direction.BACKWARD) send
							.append("MAdgb");
					else send.append("Adg0");
					send.append("Asg");
					send.append((char) ('a' + status.arm.gripper
							.getSpeed()));
				}
			...
			for (int i = 0; i < received.length - 5; i++ )
				...
				{
					status.battery.setVoltage((received[i + 2] - 'a')
							+ 0.1f * (received[i + 3] - 'a'));
					status.battery.setCurrent((received[i + 4] - 'a')
							+ 0.1f * (received[i + 5] - 'a'));
			...
			if (! status.isEmergencyStopped())
			{
				...
				for (int i = 0; i < received.length - 1; i++ )
																...
																// directions
					{
						...
						if (received[i + 3] == 'f') dir = SzarkStatus.Direction.FORWARD;
						else if (received[i + 3] == 'b') dir = SzarkStatus.Direction.BACKWARD;
						else dir = SzarkStatus.Direction.STOP;

						switch (received[i + 2])
						{
							case 's':
								status.arm.shoulder
										.setExactDirection(dir);
								break;
							case 'e':
