// changed to use with AVR-GCC by Michał Słomkowski
...
// pointer to function called after commands are received
static void (*process_function)(uint8_t*, uint8_t*);

static unsigned char TWI_buf[I2C_BUFFER_SIZE];     // Transceiver buffer. Set the size in the header file
...
start the TWI.
 ****************************************************************************/
void TWI_Slave_Initialise( unsigned char TWI_ownAddress, void (*process_func)(uint8_t*, uint8_t*))
{
...
ISR(TWI_vect)
{
	...
	// FIXME this is dirty solution, but it seems to work
	if(!TWI_Transceiver_Busy() && TWI_statusReg.RxDataInBuf)
	{
		(*process_function)(TWI_buf, TWI_buf);
