...
typedef enum { SHOULDER = 0, ELBOW, GRIPPER, WRIST } JOINT_INDEX;
...
void motor_set_direction(MOTOR motor, DIRECTION direction)
{
	...
	if( ( ((direction == MOTOR_FORWARD) || (direction == MOTOR_BACKWARD)) &&
		(joints[idx].allowedDir == 0) || (joints[idx].allowedDir == direction)))
		joints[idx].currDir = direction;
...
void motor_init()
{
	...
	joints[ELBOW].prevEncoderState = joints[ELBOW].encoderState = getval(PIN(SENSOR_ELBOW_ENC_PORT), SENSOR_ELBOW_ENC_PIN);
	joints[WRIST].prevEncoderState =joints[WRIST].encoderState = getval(PIN(SENSOR_WRIST_ENC_PORT), SENSOR_WRIST_ENC_PIN);
	joints[SHOULDER].prevEncoderState = joints[SHOULDER].encoderState = getval(PIN(SENSOR_SHOULDER_ENC_PORT), SENSOR_SHOULDER_ENC_PIN);
}
...
void motor_calibrate()
{
	...
	// shoulder
	if(bit_is_set(PIN(SENSOR_SHOULDER_MS_PORT), SENSOR_SHOULDER_MS_PIN))
		motor_set_direction(MOTOR_SHOULDER, MOTOR_BACKWARD);
	// gripper
	if(bit_is_set(PIN(SENSOR_GRIPPER_MS_PORT), SENSOR_GRIPPER_MS_PIN))
	...
	// elbow
	if(getMsState(ELBOW))
		motor_set_direction(MOTOR_ELBOW, MOTOR_BACKWARD);
	else motor_set_direction(MOTOR_ELBOW, MOTOR_FORWARD);

	// shoulder
	if(getMsState(SHOULDER))
		motor_set_direction(MOTOR_SHOULDER, MOTOR_FORWARD);
	else motor_set_direction(MOTOR_SHOULDER, MOTOR_BACKWARD);

	// gripper
	if(getMsState(GRIPPER))
		motor_set_direction(MOTOR_GRIPPER, MOTOR_FORWARD);
	...
	{
		if(joints[ELBOW].calibrated && joints[SHOULDER].calibrated && joints[WRIST].calibrated && joints[GRIPPER].calibrated) break;
	}
	...
	for(uint8_t i = 0; i < 4; i++) joints[i].mode = DIR;
...
//	if(joints[WRIST].calibrated) 
	...
	{
		if((joints[WRIST].currPos < (POS_MAX_WRIST / 2)) && !backward)
			motor_set_direction(MOTOR_WRIST, MOTOR_FORWARD);
	...
	else if(joints[WRIST].calibrated == true)
	{
		...
		else if((joints[WRIST].currPos != 0) && (joints[WRIST].currPos != POS_MAX_WRIST))
			joints[WRIST].allowedDir = 0;
	}
}
...
ISR(TIMER1_COMPA_vect)
{
	...
	else
	{
		...
		// encoders
		for(uint8_t i = 0; i < 4; i++) joints[i].prevEncoderState = joints[i].encoderState;
		...
		for(uint8_t i = 0; i < 4; i++) if(joints[i].prevEncoderState != joints[i].encoderState)
		{
			...
			if(joints[i].currDir == MOTOR_FORWARD) joints[i].currPos = joints[i].currPos >= joints[i].maxPos ? joints[i].maxPos : joints[i].currPos + 1;
			else if(joints[i].currDir == MOTOR_BACKWARD) joints[i].currPos = joints[i].currPos == 0 ? 0 : joints[i].currPos - 1;
...
static JOINT_INDEX toJoint(MOTOR motor)
{
	switch(motor)
	{
		case MOTOR_SHOULDER: return SHOULDER;
		case MOTOR_GRIPPER: return GRIPPER;	
		case MOTOR_WRIST: return WRIST; 
		case MOTOR_ELBOW: return ELBOW;
		default: return 0;
	}
}
...
static MOTOR toMotors(JOINT_INDEX index)
{
	switch(index)
	{
		case SHOULDER: return MOTOR_SHOULDER;
		case GRIPPER: return MOTOR_GRIPPER;	
		case WRIST: return MOTOR_WRIST; 
		case ELBOW: return MOTOR_ELBOW;
		default: return 0;
	}
}
